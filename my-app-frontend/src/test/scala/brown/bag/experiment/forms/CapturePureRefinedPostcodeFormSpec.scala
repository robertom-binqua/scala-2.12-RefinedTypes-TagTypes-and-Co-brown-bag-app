...
class CapturePureRefinedPostcodeFormSpec extends AnyFlatSpec {

  val postCodeFormUnderTest: Form[PureRefinedPostcode] = CapturePureRefinedPostCodeForm.form
  ...
  it should "recognised TA14 5GF as valid postcode" in {
    ...
    val maybeSomePostCode: Option[PureRefinedPostcode] = postCodeFormUnderTest
  ...
  it should "recognised a non entered postcode as invalid" in {
    ...
    formBind.errors.size should be(2)
  ...
  it should "be filled with a valid postcode" in {
    ...
    val theExpectedPostcode = PureRefinedPostcode("TA14 5GF")
